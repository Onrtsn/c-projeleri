...
namespace yeniimaj
{
    public partial class menu : Form
    {
        ...
        private void graifkler()
        {
            ...
            string query = "select " +
                 "SUM(case when TUR = 'SATIS' THEN GENELTOPLAM ELSE 0 end) as GELIR, " +
                 "SUM(case when TUR IN('GIDER', 'ALIS') THEN GENELTOPLAM ELSE 0 end) as GIDER, " +
                 "SUM(case when TUR = 'SATIS' THEN GENELTOPLAM ELSE 0 end) - SUM(case when TUR IN('GIDER', 'ALIS') THEN GENELTOPLAM ELSE 0 end) as KAR " +
                 "from MUHASEBEFIS " +
                 "WHERE TARIH BETWEEN '" + dt1.Year + "-" + dt1.Month + "-" + dt1.Day + "' AND '" + dt2.Year + "-" + dt2.Month + "-" + dt2.Day + "' ";
            ...
            string query2 = "select " +
                 " TARIH, " +
                 " SUM(case when TUR = 'SATIS' THEN GENELTOPLAM ELSE 0 end) as GELIR, " +
                 " SUM(case when TUR IN('ALIS') THEN MIKTAR ELSE 0 end) as GIDER, " +
                 " SUM(case when TUR = 'SATIS' THEN GENELTOPLAM ELSE 0 end) - SUM(case when TUR IN('GIDER', 'ALIS') THEN GENELTOPLAM ELSE 0 end) as KAR " +
                 " from MUHASEBEFIS WHERE MUHASEBEFIS.TARIH BETWEEN '" + dt1.Year + "-" + dt1.Month + "-" + dt1.Day + "' AND '" + dt2.Year + "-" + dt2.Month + "-" + dt2.Day + "' " +
